...
class WISNET(nn.Module):
    def __init__(
        ...
        # --- Hybrid Encoder (Now uses GatedInceptionWTBlock) ---
    ...
    def forward(self, x):
        ...
        if self.skip_encoder_name == 'cbam':
            ...
            # Level 1 (Stage 0): 殘差連接
            enc1 = enc1 + self.skip_encoder1(enc1) 
            # Level 2 (Stage 1): 殘差連接
        ...
        dec4 = self.wt_dec4(dec4) 

        dec3 = self.decoder3(dec4, enc2)
        dec3 = self.wt_dec3(dec3) 
...
# ==============================================================================
# 3. Hybrid Backbone (Updated with GatedInceptionWTBlock)
# ==============================================================================
class HybridBackbone(nn.Module):
    def __init__(self, in_channels, patch_size, kernel_size, exp_rate, feature_sizes, depths, drop_path_rate, use_init_weights, is_conv_stem, wave_level):
        ...
        for i in range(4):
            # 設定 wave_level (淺層細，深層粗)
            curr_wave_level = wave_level if i < 2 else 1
            
            stage = nn.Sequential(
                *[
                    InceptionWTBlock(  # <--- 使用這個新 Block
                        dim=feature_sizes[i], 
                        kernel_size=kernel_size,
                        exp_rate=exp_rate,
