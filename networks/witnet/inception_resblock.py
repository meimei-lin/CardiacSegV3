...
class INCEPTION_RESBLOCK(nn.Module):
    def __init__(
        ...
        self.decoder5 = UnetrUpBlock(spatial_dims=3, in_channels=feature_sizes[3]*2, out_channels=feature_sizes[3], kernel_size=3, upsample_kernel_size=2, norm_name=decoder_norm_name)
        self.decoder4 = UnetrUpBlock(spatial_dims=3, in_channels=feature_sizes[3], out_channels=feature_sizes[2], kernel_size=3, upsample_kernel_size=2, norm_name=decoder_norm_name)
        self.decoder3 = UnetrUpBlock(spatial_dims=3, in_channels=feature_sizes[2], out_channels=feature_sizes[1], kernel_size=3, upsample_kernel_size=2, norm_name=decoder_norm_name)
        self.decoder2 = UnetrUpBlock(spatial_dims=3, in_channels=feature_sizes[1], out_channels=feature_sizes[0], kernel_size=3, upsample_kernel_size=2, norm_name=decoder_norm_name)
        self.decoder1 = UnetrUpBlock(spatial_dims=3, in_channels=feature_sizes[0], out_channels=first_feature_size, kernel_size=3, upsample_kernel_size=2, norm_name=decoder_norm_name)
        
        self.out_block = UnetOutBlock(spatial_dims=3, in_channels=first_feature_size, out_channels=out_channels)
        
        self.deep_sup = deep_sup
        if deep_sup:
            self.ds_block1 = UnetOutBlock(spatial_dims=3, in_channels=feature_sizes[0], out_channels=out_channels) 
            self.ds_block2 = UnetOutBlock(spatial_dims=3, in_channels=feature_sizes[1], out_channels=out_channels)    
...
class Backbone(nn.Module):
    def __init__(self, in_channels, patch_size, kernel_size, exp_rate, feature_sizes, depths, drop_path_rate, use_init_weights, is_conv_stem):
        super().__init__()
